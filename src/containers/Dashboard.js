...
export default class {
	...
	handleEditTicket(e, bill, bills) {		
		...
		if (this.counter % 2 === 0) bills.forEach((b) => $(`#open-bill${b.id}`).css({ background: "#0D5AE5" }));

		$(`#open-bill${bill.id}`).css({ background: (this.counter % 2 === 0) ? "#2A2B35" : "#0D5AE5" });
		$(".dashboard-right-container div").html((this.counter % 2 === 0) ? DashboardFormUI(bill) : `<div id="big-billed-icon" data-testid="big-billed-icon"> ${BigBilledIcon} </div>`);
		$(".vertical-navbar").css({ height: (this.counter % 2 === 0) ? "150vh" : "120vh" });
	...
	handleShowTickets(e, bills, index) {		
		...
		$(`#arrow-icon${this.index}`).css({ transform: (this.counter % 2 === 0) ? "rotate(0deg)" : "rotate(90deg)" });
		$(`#status-bills-container${this.index}`).html((this.counter % 2 === 0) ? cards(filteredBills(bills, getStatus(this.index))) : "");
	...
	updateBill = (bill) => {
		if (this.store) {
			return this.store.bills().update({ data: JSON.stringify(bill), selector: bill.id }).then((bill) => bill).catch(console.log);
		}
	};
}